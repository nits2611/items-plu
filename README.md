...
v52.2.1 introduces a canonical `images` array for products. CSV and Google Sheets store the array as JSON text, while IndexedDB stores it as a normal JavaScript array. Existing `image_url` and `image_local` fields remain readable for backward compatibility. Tapping a product thumbnail opens the shared responsive image gallery.
